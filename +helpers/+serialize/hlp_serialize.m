...
%                                (C) 2010 Tim Hutt
%
%                                Changed the data type from uint8 to int8
...
function m = serialize_scalar(v)
    % Data type & data
    m = [class2tag(class(v)); typecast(v,'int8').'];
end
...
function m = serialize_numeric_simple(v)
    % Tag & Number of dimensions, Dimensions, Data
    m = [16+class2tag(class(v)); ndims(v); typecast(uint32(size(v)),'int8').'; typecast(v(:).','int8').'];
end
...
function m = serialize_cell(v)
    ...
    if sizeprod == 1
        ...
        if (all(cellfun('isclass',v(:),'double')) || all(cellfun('isclass',v(:),'single'))) && all(~cellfun(@issparse,v(:)))
            ...
            if reality
                % all real
                m = [int8(34); serialize_numeric_simple(reshape([v{:}],size(v)))];
            elseif ~reality
                % all complex
                m = [int8(34); serialize_numeric(reshape([v{:}],size(v)))];
            else
                % mixed reality
                m = [int8(35); serialize_numeric(reshape([v{:}],size(v))); serialize_logical(reality(:))];
            end
        else
            % non-float types
            ...
            elseif cellfun('isclass',v,'logical')
                % bool flags
                m = [int8(39); serialize_logical(reshape([v{:}],size(v)))];
    ...
    elseif isempty(v)
        % empty cell array
        m = [int8(33); ndims(v); typecast(uint32(size(v)),'int8').'];
    else        
        ...
        if cellfun('isclass',v,'char') & size1 <= 1 %#ok<AND2>
            ...
            m = [int8(36); serialize_string([v{:}]); serialize_numeric_simple(uint32(size2)); serialize_logical(size1(:)==0)];
